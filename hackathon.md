...
## 20 Sep, late afternoon: judge simulation, two approved specs, and corrected claims

An outside-eye pass: Codex GPT-6 Astra scored the entry as a judge would (works live 8, usefulness 7, sponsor depth 8.5, code 7, presentation 6.5), then compared it with the strongest public entries on vibeapps; the decision model was asked, with the same material, which change mattered most. Both put presentation first, and both ranked two product changes worth doing. The owner approved them as SPEC-016 and SPEC-017.

- **SPEC-016, demo first on an empty board.** A first-time visitor's board opens on a demo card with an optional alert-email field and one primary button; the URL form sits below. Once a page exists the card collapses to the old one-line hint. The demo row carries a four-step progress line: original captured, edit published, judged (with the importance label), emailed or not emailed with the reason (`src/App.tsx`).
- **SPEC-017, noise then signal.** The demo has three phases. The new middle step, "Publish a cosmetic edit", rewords one sentence and bumps the timestamp and visitor counter; the pipeline logs it as cosmetic and sends nothing, and the row says so. Then the fee and deadline change produces the alert. A visitor may skip straight to the fee change (`convex/demo.ts`, `convex/watches.ts`). Existing finished demos were mapped to the new final phase.
- **Claims corrected after the review.** "Any web page" is now "public web pages"; "exact diff" is the diff of the captured, normalised text; the filter is described as a judgment with smoke tests, not a guarantee; inbound mail with no authentication results is now ignored rather than allowed, so "must pass SPF or DKIM" is true as written; snapshot and change writes carry the check's claim token, so a superseded worker cannot write a snapshot or a change, which the earlier entry claimed before it was fully true (`convex/email.ts`, `convex/watches.ts`, `convex/checks.ts`, `README.md`).
- **Verification.** `scripts/noise-then-signal.mjs`: fresh guest, demo card first, cosmetic edit judged cosmetic and not emailed, fee change judged "money, dates or availability" and emailed to a saved address, no errors. Late-publish and chaos runs clean. A captioned v3 video was recorded from the live app for the owner to upload.

## 20 Sep, evening: a real-page matrix found a rate-limit bug

Twenty-two pages of different kinds (government portals, a status page, GitHub releases, an Arabic news site, exchange rates, a rankings page, sites with bot walls, a very noisy feed) were added to one board at once. Thirteen failed with HTTP 429: not the sites, but Firecrawl rate-limiting our own burst. The row blamed the site ("it may block automated readers") and the page was benched for six hours. A judge pasting five pages quickly would have hit the same wall.

Fixed: at most three scrapes run at once across the deployment (`claimCheck` returns "busy" and the check retries after a few seconds, bounded); the cron staggers its due checks; a 429 or rate-limit error is treated as transient, keeps the page's status and retries in about two minutes; the error backoff for genuine failures is now at least 30 minutes rather than six hours; the wording says "Too many checks at once, Pigeon will try again in a couple of minutes." After the fix 18 of 22 pages scrape cleanly; the four that fail are genuine (bot walls and a 404). Three changes were detected in the first hour and all three were correctly judged cosmetic: a headline reshuffle, a translation widget disappearing, a feed reordering. One observation for a later spec: a summary of an Arabic page was written in Arabic (`convex/watches.ts`, `convex/checks.ts`, `convex/admin.ts`, `scripts/add-many.mjs`, `scripts/matrix-report.mjs`).

## 21 Sep: Firecrawl credits are monthly, not daily

Firecrawl's usage email showed 50% of the free tier's 1,000 credits used after two days. The credits are per month (renewing 19 Oct), and the deployment's guard was 700 a day, sized as if they were daily. Every scrape so far was our own testing (chaos runs, judge-path replays, the 22-page matrix); the usage table shows 378 on 20 Sep and 146 on 21 Sep, and no board on the deployment belongs to anyone else yet.

Response: every existing watch paused (all are test artefacts); the daily guard lowered to 40; the per-board judge demo exempted from the guard so a judge can never be refused; README corrected. Remaining credits are reserved for judging (`convex/watches.ts`, `convex/admin.ts`, `README.md`).
