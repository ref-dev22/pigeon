...
const showRow = async () => { await p.locator("[data-pigeon-progress]").first().evaluate((el) => el.scrollIntoView({ block: "center", behavior: "smooth" })); await hold(1.5); };
const showChanges = async () => { await p.locator("text=What changed").first().evaluate((el) => el.scrollIntoView({ block: "start", behavior: "smooth" })); await hold(1.5); };
await showRow();
...
await caption("The decision model judges it cosmetic. No email. That is the point."); await hold(20); await showChanges(); await hold(5); await showRow();
...
await caption("Two models: one judges importance with probabilities, one writes two sentences."); await hold(22); await showChanges(); await hold(3);
await caption("Money, dates or availability. Emailed."); await hold(4);
const diff = p.locator("a", { hasText: /see diff/i }).first();
await diff.click(); await p.waitForURL(/#\/change\//, { timeout: 10000 }).catch(() => {}); await hold(1);
